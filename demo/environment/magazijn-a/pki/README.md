...
- Peer-OIN = Peer ID = `serialnumber` in elke `csr.json`: `00000000000000100000`.
- `names[].O`: `magazijn-a`.
...
```bash
openssl x509 -in out/magazijn-a/inway/cert.pem -noout -subject | grep 00000000000000100000
```
...
for f in pki/peers/magazijn-a/*/csr.json; do
  jq -e '.serialnumber=="00000000000000100000"' "$f" >/dev/null && echo "$f OK"
done
