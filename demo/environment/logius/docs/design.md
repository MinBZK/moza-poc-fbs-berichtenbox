...
| controller | `logius-fscctl` | beheer-UI: afnemer-toegang aanvragen + contracten beheren/inspecteren (Administration/Registration-API, `AUTHN_TYPE=none` — op ZAD afgeschermd door de `authorization-wall` vóór het component, zie hoofdstuk 10 van `demo/environment/zad-demo/README.md`); `controller-migrate`-wrapper migreert bij boot. |
