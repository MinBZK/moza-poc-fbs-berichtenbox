 ...
 */
class AanleverServiceHaperingTest {
    ...
    fun `een afgekapt antwoord laat de ronde doorlopen`() {
        ...
        val resultaat = service(mapOf(OIN_A to magazijn)).leverAan(ronde(3))

        assertEquals(uitkomst(aangeboden = 3, geslaagd = 3, zonderBerichtId = 1), tellers(resultaat))
    ...
    fun `een berichtId zonder inhoud telt als afwezig en levert geen markeer-aanroep op`(leeg: String) {
        ...
        val resultaat = service(mapOf(OIN_A to magazijn)).leverAan(ronde(3, gelezen = true))
    ...
    fun `een onbruikbaar antwoord telt één keer, ook wanneer om gelezen gevraagd was`() {
        ...
        val resultaat = service(mapOf(OIN_A to magazijn)).leverAan(ronde(1, gelezen = true))

        assertEquals(uitkomst(aangeboden = 1, geslaagd = 1, zonderBerichtId = 1), tellers(resultaat))
    ...
    fun `een antwoord wordt gesloten ook als het uitlezen gooit`() {
        ...
        service(mapOf(OIN_A to magazijn({ kapot }))).leverAan(ronde(1))
    ...
    fun `een antwoord dat niet te sluiten is laat de ronde doorlopen`() {
        ...
        assertEquals(uitkomst(aangeboden = 3, geslaagd = 3), tellers(resultaat))
        verify(exactly = 3) { magazijn.leverAan(any()) }
    }
    ...
    fun `een antwoord wordt ook gesloten als alles goed gaat`() {
        ...
        service(mapOf(OIN_A to magazijn({ geslaagd }))).leverAan(ronde(1))
    ...
    fun `het antwoord op een geweigerde markering wordt gesloten`() {
        ...
        service(mapOf(OIN_A to magazijn)).leverAan(ronde(1, gelezen = true))
    ...
    fun `een onbereikbaar magazijn laat de ronde doorlopen`() {
        ...
        assertEquals(uitkomst(aangeboden = 3, geslaagd = 2, mislukt = 1), tellers(resultaat))
        verify(exactly = 3) { magazijn.leverAan(any()) }
    }
    ...
    fun `een opdracht voor een onbekend magazijn telt als mislukt en laat de rest doorlopen`() {
        ...
        val resultaat = service(mapOf(OIN_A to magazijn)).leverAan(opdrachten)
    ...
    fun `elke opdracht gaat naar het magazijn van zijn eigen OIN`() {
        ...
        val resultaat = service(mapOf(OIN_A to magazijnA, OIN_B to magazijnB))
            .leverAan(opdrachten)
    ...
    fun `een geweigerde markering telt als mislukte markering en niet als mislukt bericht`() {
        ...
        assertEquals(uitkomst(aangeboden = 1, geslaagd = 1, markeringMislukt = 1), tellers(resultaat))
    }
    ...
    fun `twee onbruikbare antwoorden in één ronde tellen allebei`() {
        ...
        assertEquals(uitkomst(aangeboden = 3, geslaagd = 3, zonderBerichtId = 2), tellers(resultaat))
    }
    ...
    fun `geen enkele logregel draagt de waarde van de ontvanger`() {
        ...
        val magazijn = magazijn(
            ...
            // Twee regels: de afwijzing zelf, en de reden die er niet uit te lezen viel.
            { onleesbareAfwijzing(ProcessingException("detail over BSN:999999011 niet te lezen")) },
            geldig(8),
            geldig(9),
        )
        ...
        val opdrachten = ronde(7) + opdracht(8, gelezen = true) + opdracht(9, gelezen = true) +
            opdracht(10).copy(magazijnOin = ONBEKENDE_OIN)

        val regels = vangLogregels { service(mapOf(OIN_A to magazijn)).leverAan(opdrachten) }

        assertEquals(11, regels.size, "elke onderdrukte fout hoort een regel op te leveren")
    ...
    fun `de logregel noemt waarom het berichtId ontbrak`() {
        ...
        val bijAfgekapt = vangLogregels { service(mapOf(OIN_A to afgekapt)).leverAan(ronde(1)) }
        val bijLeeg = vangLogregels { service(mapOf(OIN_A to leeg)).leverAan(ronde(1)) }
    ...
    fun `de logregel draagt de hele oorzaakketen en niet alleen de bovenste fout`() {
        ...
        val regels = vangLogregels { service(mapOf(OIN_A to magazijn)).leverAan(ronde(1)) }
    ...
    fun `een fout die zichzelf als oorzaak noemt levert één naam op`() {
        ...
        val regels = vangLogregels {
            service(mapOf(OIN_A to magazijn({ throw ZichzelfAlsOorzaak() }))).leverAan(ronde(1))
        }
    ...
    fun `een fout die geen magazijnstoring is klinkt luider`() {
        ...
        val storingsregels = vangLogregels { service(mapOf(OIN_A to storing)).leverAan(ronde(1)) }
        val bedradingsregels = vangLogregels { service(mapOf(OIN_A to bedrading)).leverAan(ronde(1)) }
    ...
    fun `een afgekapte stream is een hapering, een antwoord dat al gelezen was niet`() {
        ...
        val bijAfgekapt = vangLogregels { service(mapOf(OIN_A to afgekapt)).leverAan(ronde(1)) }
        val bijAlGelezen = vangLogregels { service(mapOf(OIN_A to algelezen)).leverAan(ronde(1)) }
    ...
    fun `een geweigerde aanlevering klinkt luider dan een magazijn dat het even niet aankan`() {
        ...
        val bijGeweigerd = vangLogregels { service(mapOf(OIN_A to geweigerd)).leverAan(ronde(1)) }
        val bijOverbelast = vangLogregels { service(mapOf(OIN_A to overbelast)).leverAan(ronde(1)) }
    ...
    fun `een luide logregel wijst aan waar de fout ontstond`() {
        ...
        assertTrue(
            Regex(""" @ \S*AanleverServiceHaperingTest\S*\.\S+:\d+""").containsMatchIn(regels.single().message),
    ...
    fun `een opdracht voor een onbekend magazijn klinkt als inrichtingsfout`() {
        val regels = vangLogregels {
            service(mapOf(OIN_A to magazijn())).leverAan(listOf(opdracht(1).copy(magazijnOin = ONBEKENDE_OIN)))
        }
    ...
    fun `een fout zonder eenvoudige naam wordt alsnog benoemd`() {
        ...
        val regels = vangLogregels {
            service(mapOf(OIN_A to magazijn({ throw naamloos }))).leverAan(ronde(1))
        }
    ...
    fun `een lege ronde levert een lege uitkomst`() {
        ...
        val resultaat = service(mapOf(OIN_A to magazijn)).leverAan(emptyList())

        assertEquals(uitkomst(aangeboden = 0), tellers(resultaat))
    ...
    fun `een ronde van één bericht slaagt`() {
        ...
        val resultaat = service(mapOf(OIN_A to magazijn)).leverAan(ronde(1))

        assertEquals(uitkomst(aangeboden = 1, geslaagd = 1), tellers(resultaat))
        verify(exactly = 0) { magazijn.markeer(any(), any(), any()) }
    }
    ...
    private companion object {
        ...
        fun antwoord(status: Int, entity: AanleverRespons?): Response {
            ...
            // Zonder body: bij een 4xx vraagt de service eerst of er een problem+json te lezen valt.
            every { response.hasEntity() } returns false
        ...
        /** Een afwijzing mét body, waar de reden van het magazijn niet uit te lezen is. */
        fun onleesbareAfwijzing(fout: Throwable): Response {
            val response = mockk<Response>()

            every { response.status } returns 403
            every { response.hasEntity() } returns true
            every { response.readEntity(Problem::class.java) } throws fout
            justRun { response.close() }
        ...
        /** De service tegen deze magazijnen; een OIN die er niet in staat, heeft geen adres. */
        fun service(magazijnen: Map<String, MagazijnAanleverClient>): AanleverService {
            val clients = mockk<MagazijnClients>()

            every { clients[any()] } answers { magazijnen[firstArg()] }

            return AanleverService(clients)
        }

        /** Alleen de tellers: welke reden er bij een mislukking hoort, toetst [AanleverServiceTest]. */
        data class Tellers(
            val aangeboden: Int,
            val geslaagd: Int,
            val mislukt: Int,
        ...
        ) = Tellers(aangeboden, geslaagd, mislukt, markeringMislukt, zonderBerichtId)

        fun tellers(resultaat: AanleverResultaat) = with(resultaat) {
            Tellers(aangeboden, geslaagd, mislukt, markeringMislukt, zonderBerichtId)
        }
    }
}
