 ...
 * niets van te merken. Zou de simulator zonder configuratie tóch iets verzinnen, dan bepaalt hij wie
 * er in de demo meespeelt, en dat is niet aan hem: de ondernemers komen uit hetzelfde
 * generator-artefact als de magazijnenset.
 *
 * Draait bewust op het gewone testprofiel, want dát is het profiel zonder opstartvulling.
 */
@QuarkusTest
class OpstartvullingZonderConfiguratieTest : MagazijnTestBasis() {
    ...
    fun `zonder ontvangers blijft een lege opslag leeg`() {
        assertEquals(Opstartvulling.Uitkomst.NietGeconfigureerd, opstartvulling.vulOntbrekende())
        assertTrue(bulk.magazijnenMetBerichten().isEmpty(), "er hoort niets klaargezet te zijn")
    }
}
