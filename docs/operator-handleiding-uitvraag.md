...
| `MAGAZIJN_<X>_NAAM` | Weergavenaam per deelnemende organisatie, onder `magazijnen."<OIN>".naam`; die naam ziet de ondernemer bij elk bericht in zijn lijst | **Verplicht.** Een ontbrekende naam laat de config-binding al falen, een lege of alleen-whitespace naam wordt door `ConfigMagazijnregister` geweigerd. Beide blokkeren de start — liever dat dan een berichtenlijst met een lege afzenderkolom |
