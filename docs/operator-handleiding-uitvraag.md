...
| `magazijnen."<OIN>".naam` | Weergavenaam van de organisatie; die naam ziet de ondernemer bij elk bericht in zijn lijst. Geen omgevingsvariabele maar een configuratieregel per magazijn | **Verplicht.** Een ontbrekende naam laat de config-binding falen (`SRCFG00014`), een lege of alleen-whitespace naam wordt door `ConfigMagazijnregister` geweigerd. Beide blokkeren de start — liever dat dan een berichtenlijst met een lege afzenderkolom |
