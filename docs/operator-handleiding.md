...
| `LDV_POSTGRES_URL` | env var | JDBC-URL van het logboek; buiten dev/test verplicht `ssl=true` of `sslmode=require`/`verify-ca`/`verify-full` conform BIO 13.2.1 | Geen default in `%prod`, en `LdvEndpointValidator` leest de waarde bij boot: ontbreekt de env var of versleutelt de URL niet, dan start de service niet |
