...
- **Alles hangt onder epic [#238](https://github.com/MinBZK/MijnOverheidZakelijk/issues/238)**, en daarbinnen onder de groep-issue waar het werk bij hoort: [#787](https://github.com/MinBZK/MijnOverheidZakelijk/issues/787) Demo Federatief Berichtenstelsel — **alles wat de demo doet of moet kunnen tonen**: simulatie-engine en storingsscenario's, berichtenbox-UI, demo-omgeving (laptop, ZAD, proeftuin, bedieningspaneel, dataset, persona's), runbook, én vraagstukken die de demo bespreekbaar moet maken ook als er nog geen oplossing is. Staat in de aanleiding "de demo moet dit kunnen laten zien of bespreekbaar maken", dan hoort het hier — dat gaat vóór #349. [#552](https://github.com/MinBZK/MijnOverheidZakelijk/issues/552) authenticatie/autorisatie, [#947](https://github.com/MinBZK/MijnOverheidZakelijk/issues/947) Model AppManager, [#349](https://github.com/MinBZK/MijnOverheidZakelijk/issues/349) PoC als restcategorie voor productcode, keten, CI/CD, deploy en documentatie — niet de standaardkeuze bij twijfel. Past het bij geen enkele groep: rechtstreeks onder #238, en meld dat bij het opleveren. Koppel via de sub-issue-relatie, niet via een `> Onderdeel van #N.`-regel in de tekst — `gh` (2.46) kent daar geen commando voor, dus via GraphQL:
...
gh api graphql -f query='mutation($p:ID!,$c:ID!){addSubIssue(input:{issueId:$p,subIssueId:$c}){subIssue{number}}}' \
  -f p="$(issue_id 787)" -f c="$(issue_id <n>)"
```
