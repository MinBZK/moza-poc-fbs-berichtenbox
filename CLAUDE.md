...
- **Claude Code-automatisering** (ingecheckt in `.claude/`, dus teambreed): twee PreToolUse-guards blokkeren een edit in `target/generated-sources` en in een bestaande Flyway-migratie; een PostToolUse-hook leidt uit het bewerkte pad de module af en draait de tests van de twee pure-JVM-libraries meteen (`FBS_HOOK_TESTS=0` zet dat uit). Subagents: `taal-en-commentaar-reviewer` (NL/EN-grens en commentaarregels), `pii-log-auditor` (BSN-lekken, en OIN juist níet maskeren), `security-reviewer` (OWASP). Skills: `/openapi-wijziging`, `/pr-klaarmaken`, `/dev-check`. Hernoem je een module, werk dan de lijst in `.claude/hooks/geraakte-module-test.sh` bij — de vorige hook noemde één hardgecodeerd pad en verstomde stil toen dat pad verdween.
