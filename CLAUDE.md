...
- **detekt (Kotlin static analysis):** `detekt-maven-plugin` aan `verify` gebonden; rulesets `complexity`, `potential-bugs`, `exceptions` en `empty-blocks` (zie `detekt.yml`) — de correctness-/robustness-assen die CodeQL (zwakker voor Kotlin) en JaCoCo niet borgen; overlap met CodeQL bewust geaccepteerd (extra guardrails bij AI-ondersteunde ontwikkeling). Stijl/naming/comments staan uit. Lokaal draaien: `./mvnw detekt:check`. Gate: `maxIssues: 0` + per-module `detekt-baseline.xml` — bestaande debt staat in de baseline en faalt niet, élke nieuwe bevinding faalt wel (baseline stapsgewijs naar 0). Schone modules + de root-POM hebben een lege baseline omdat de plugin het bestand verplicht stelt. Nieuwe baseline genereren: `./mvnw detekt:create-baseline`. SARIF gaat in CI (`.github/workflows/detekt.yml`) naar het code-scanning-dashboard. Test-sources zijn uitgesloten.
