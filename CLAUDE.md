...
De volledige debug-gids staat in [`docs/operations/zad-gitops.md`](docs/operations/zad-gitops.md):
de vijf ZAD-eigenschappen die bepalen wat een component wél en niet kan, de drie GitOps-lagen met
de grond-waarheid per laag, de `zadctl`-commando's (incl. inloggen vanuit een container) en de
valkuilen. Lees dat bestand vóór je een falende deploy of een vastgelopen preview onderzoekt.

Wat je hoe dan ook moet weten:

- **Git is de bron van waarheid, niet de cluster.** Argo-Applications hebben `selfHeal: true` +
...
| `libraries/fbs-common/`                | Gedeeld cross-cutting: JAX-RS filters, exception mappers, `identificatie/` (BSN/RSIN/OIN), `profiel/` (Profiel-serviceclient, voorkeuren, toestemming), `fsc/` (outway-headers, outbound-TLS) en de LDV-validators |
...
- **detekt (Kotlin static analysis):** `detekt-maven-plugin` aan `verify` gebonden; rulesets `complexity`, `potential-bugs`, `exceptions`, `empty-blocks` plus een curated set `style`/`naming` (zie `detekt.yml`) — de correctness-/robustness-assen die CodeQL (zwakker voor Kotlin) en JaCoCo niet borgen; overlap met CodeQL bewust geaccepteerd (extra guardrails bij AI-ondersteunde ontwikkeling). De `comments`-ruleset staat uit (botst met "comment het waarom, niet het wat"). Lokaal draaien: `./mvnw detekt:check`. Gate: `maxIssues: 0` ZONDER baseline — élke bevinding faalt de build en wordt écht opgelost; bewuste, onvermijdelijke uitzonderingen krijgen een inline `@Suppress("Rule")` met motivatie-comment (zichtbaar in review, geen verborgen debt). SARIF gaat in CI (`.github/workflows/detekt.yml`) naar het code-scanning-dashboard. Test-sources zijn uitgesloten.
- **Coverage op PR:** `madrapps/jacoco-report` (GitHub-native, SHA-pinned in `test.yml`) plaatst de JaCoCo-coverage-diff als PR-comment uit de bestaande `jacoco.xml`-rapporten. Bewust géén Codecov (externe SaaS + upload-token + supply-chain-oppervlak). De harde 90%-gate blijft de JaCoCo `check` in de build; de comment is informatief. **Lees `target/site/jacoco/jacoco.xml`** (jacoco-maven-plugin, mét `api.**`/`common.**`-excludes — spiegelt de gate), niet `target/jacoco-report/jacoco.xml` (auto-rapport van de `quarkus-jacoco`-extensie: alleen `@QuarkusTest`-data, géén excludes → gegenereerde code drukt het cijfer kunstmatig). Pad globt op `services/*` én `libraries/*` zodat nieuwe modules automatisch meetellen. `fbs-common` heeft een eigen `jacoco-maven-plugin` (prepare-agent + report + 90%-`check`); het is een pure-JVM-library zonder `@QuarkusTest`, dus de standaard jacoco-agent op Surefire volstaat (geen `quarkus-jacoco`).
- **CI-status volgen:** `gh pr checks <PR#>` en `gh run watch <run-id> --exit-status`. Bij falen: `gh run view <id> --log-failed`.
- **Claude Code-automatisering** (ingecheckt in `.claude/`, dus teambreed). Hooks: vier PreToolUse-guards blokkeren een edit in `target/generated-sources`, een edit in een bestaande Flyway-migratie, een nieuw pad met witruimte erin, en een `git push` waarvan de bestemming `main` is (`FBS_HOOK_MAIN_PUSH=0` zet die laatste uit). Drie PostToolUse-hooks draaien na een Kotlin-edit de tests van de geraakte module, na een edit onder `.github/scripts/` de bijbehorende `test-*.sh`, en melden na een spec-edit welke paden nog geen Bruno-request hebben (`FBS_HOOK_TESTS=0` zet het testdeel uit). Een SessionStart-hook meldt een bezette poort 8081 — dan faalt élke `@QuarkusTest`, en de melding noemt de oorzaak niet. Subagents: `taal-en-commentaar-reviewer` (NL/EN-grens en commentaarregels), `pii-log-auditor` (BSN-lekken, en OIN juist níet maskeren), `kotlin-stijl-reviewer` (de lege-regel-conventies, die detekt niet dekt), `migratie-reviewer` (de database-conventies), `ci-workflow-reviewer` (de Actions-valkuilen die groen falen), `openapi-adr-reviewer` (wat Spectral niet dekt), `test-dekkings-reviewer` (inputvariatie en de coverage-gate), `security-reviewer` (OWASP). Skills: `/openapi-wijziging`, `/pr-klaarmaken`, `/dev-check`, `/nieuw-issue`, `/nieuwe-migratie`, `/zad-debug`, `/demo-starten`, `/warnings-triage`. Hernoem je een module, werk dan de lijst in `.claude/hooks/geraakte-module-test.sh` bij — de vorige hook noemde één hardgecodeerd pad en verstomde stil toen dat pad verdween.
